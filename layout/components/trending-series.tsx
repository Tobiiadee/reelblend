...
export default function TrendingSeries() {
  const { showLeft, showRight, scrollLeft, scrollRight, scrollRef } = useScroll();

  const { data: trendingSeries, isLoading, error } = useQuery({
        ...
        >
          {isLoading
            ...
            : trendingSeries?.results.map(
                ({ id, original_name, first_air_date, poster_path, vote_average }) => (
                  <MovieCard
                    key={id}
                    id={id}
                    title={original_name}
